...
def initialClique(myArr, myActSortByCent, iterations, iterMax):
    ...
    for acteur in randrange(0, nrOfActors - 1):
        if len(randClique) == 0:
            ...
            if acteur in randClique:
                continue          
            for j in range(len(randClique)):
                # auf Gegenseitigkeit pruefen
                if ((myArr[randClique[j]][acteur] == 0) or (myArr[acteur][randClique[j]] == 0)):
                    break
            else:
                # Only executed if the loop wasn't broken (all connections exist)
                randClique.append(acteur)
    return randClique
...
def neighborhoodAddSwap(myArr, myClique, myActSortByCent, tabuDROPList, tabuSWAPList, iterations, iterMax):
    ...
    nrOfActorsPos = len(myActSortByCent)
    ...
    if len(addCandidates) > 0:
        addSum = 0
        maxAddSum = 0
        for testCand in range(len(addCandidates)):
            for testCandCompare in range(len(bestCliqueAddCand)):
    ...
    if len(swapCandidates) > 0 and cliqueSize > 1:
        swapSum = 0
        swapNeg = 0
        maxSWAPDiff = -sys.maxsize
        for testCand in range(len(swapCandidates)):
            candToSwap = swapCandDict[swapCandidates[testCand]]
            for testCandCompare in range(len(bestCliqueSwapCand)):
...
def select_best_neighbor(myArr, currentClique, myActSortByCent, tabuDROPList, tabuSWAPList, tabuADDList, tabuADDlength, tabuDROPlength, iterations, iterMax):
    ...
    # Select best option
    if len(bestAddCliqueCand) > 0 and weightOfClique(myArr, bestAddCliqueCand) > weightOfClique(myArr, bestSwapCliqueCand):
        # ADD selected
        bestNeighborClique = bestAddCliqueCand[:]
        manage_tabu_list(tabuADDList, tabuADDlength, tabuADDCand)
        return bestNeighborClique
    elif weightOfClique(myArr, bestDropCliqueCand) > weightOfClique(myArr, bestSwapCliqueCand):
        # DROP selected
        bestNeighborClique = bestDropCliqueCand[:]
        manage_tabu_list(tabuDROPList, tabuDROPlength, tabuDropCand)
        return bestNeighborClique
    elif len(bestSwapCliqueCand) > 0:
        # SWAP selected
...
def manage_swap_tabu_list(tabuSWAPList, nrOfSWAPCand):
    """Manage SWAP tabu list with dynamic length based on candidates"""
